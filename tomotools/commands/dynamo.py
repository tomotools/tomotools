...
class DataDir:
    ...
    def path_for_tag(self, tag: int):
        ...
        batch_dir: str = f'batch_{tag // batch_size * batch_size}' if batch_size > 0 else ''
    ...
    def write_table(self, path):
        dynamotable.write(self.table[[col for col in TABLE_COLUMN_NAMES if col in self.table.columns]], path)

    def link_particles_to(self, path):
        def link_tag(row):
            tag = row['tag']
            symlink(row['particle_path'], join(path, f'particle_{tag:0{padding}}.{ext}'))
...
def _link_particles(source_dbox: DBox, dest_dbox: DBox):
    ...
    for batch_dir in [dir for dir in listdir(source_dbox.path) if dir.startswith('batch_')]:
        ...
        for particle in listdir(join(source_dbox.path, batch_dir)):
            symlink(join(source_dbox.path, batch_dir, particle),
...
def _merge_tags(original, appended):
    ...
    data = np.array([[np.append(original[1], appended[1] + np.max(original[1], initial=0))]])
...
def _decimal_digits(number: int) -> int:
    if number < 1:
        raise ValueError('Number of decimal places will only be determined for numbers > 0')
    digits: int = 0
    ...
    for dbox in dboxes:
        ...
        for tag in dbox.table['tag']:
            ...
            if isfile(dst_particle):
                raise FileExistsError(f'Destination particle file already exists: {dst_particle}')
