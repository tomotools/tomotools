...
def cryocare_extract(
    ...
    for input_file in input_files:
        ...
        if input_file.is_file():
            ...
            input_tomo += [
                Tomogram(Path(file))
                for file in glob(join(input_file, "*_rec_bin_[0-9].mrc"))
            ]

    for tomo in input_tomo:
        if path.isfile(
            tomo.path.with_name(f"{tomo.path.stem}_EVN.mrc")
        ) and path.isfile(tomo.path.with_name(f"{tomo.path.stem}_ODD.mrc")):
            tomo = tomo.with_split_files(
                tomo.path.with_name(f"{tomo.path.stem}_EVN.mrc"),
                tomo.path.with_name(f"{tomo.path.stem}_ODD.mrc"),
            )
