...
def frames2stack(
    ...
    if not skip_evnodd and all(subframe.is_split for subframe in subframes):
        stack_subframes_pairs += [
            (
                f"{full_stack_basename}_EVN{full_stack_ext}",
                [subframe.path_evn for subframe in subframes],
            ),
            (
                f"{full_stack_basename}_ODD{full_stack_ext}",
                [subframe.path_odd for subframe in subframes],
            ),
        ]
    ...
    for partial_stack_path, partial_stack_subframes in stack_subframes_pairs:
        subprocess.run(
            ["newstack"] + partial_stack_subframes + [partial_stack_path] + ["-quiet"]
        )
        ...
        with mrcfile.mmap(partial_stack_path, "r+") as mrc:
            ...
            for i in range(10):
                title = (
                    stack_mdoc["titles"][i].encode()
                    if i < len(stack_mdoc["titles"])
                    else b""
                )
...
def motioncor2(
    ...
    if gain_ref_dm4 is not None:
        ...
        gain_ref_mrc = splitext(basename(gain_ref_dm4))[
            0
        ]  # Basename of gain ref without extension and path
    ...
    for subframe in subframes:
        if subframe.subframe_mdoc:
            ...
            mdocfile.write(
                ...
                join(
                    output_dir,
                    splitext(splitext(basename(subframe.mdoc_path))[0])[0]
                    + ".mrc.mdoc",
                ),
            )
    ...
    output_frames = [
        SubFrame(
            path=join(output_dir, splitext(basename(subframe.path))[0] + ".mrc"),
            tilt_angle=subframe.tilt_angle,
        )
        for subframe in subframes
    ]
