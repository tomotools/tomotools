...
class TiltSeries:
    ...
    def is_split(self) -> bool:
        """Return whether the tilt series is split into even/odd files."""
        if self._is_split is None:
            self._is_split = self.find_split_files()
        return self._is_split

    @staticmethod
    def from_path(path: Path) -> Iterator["TiltSeries"]:
    ...
    def _from_dir(path: Path) -> Iterator["TiltSeries"]:
        """Create TiltSeries from a directory containing mrc/st files."""
    ...
    def find_split_files(self) -> bool:
        ...
        Searches for even/odd split files in:
        1. The same directory as the mrc file, suffixed with _EVN/_ODD
        2. The same directory as the mrc file, suffixed with _even/_odd
        """
        for even, odd in [
            (
