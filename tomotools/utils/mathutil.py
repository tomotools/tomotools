...
def wiener(angpix, defocus, snrfalloff, deconvstrength, hpnyquist, phaseflipped, phaseshift):
    ...
    snr = np.exp(np.linspace(0,-1,2048)*snrfalloff*100/angpix)*np.power(10,3*deconvstrength)*highpass
    
    ctf = tom_ctf1d(2048, angpix*1e-10, 300e3, 2.7e-3, -1*defocus*1e-6, 0.07, phaseshift / 180 * np.pi, 0)
