...
class Micrograph:
    ...
    def from_movies(
        ...
        if gain_ref_dm4 is not None and gain_ref_mrc is None:
            ...
            print(
                f"Found unique gain reference {gain_ref_dm4}, converting to MRC at {gain_ref_mrc}"
            )
        ...
        for movie in movies:
            if movie.mdoc is not None:
                ...
                if not (
                    ...
                    raise Exception(
                        "Unexpected MDOC format: tomotools can only handle a single frameset per mdoc"
                    )
