...
def fix_tiltcom(
    ...
    modify_value(ts.path.with_name("tilt.com"), "InputProjections", f"{ts.path.name}")
    modify_value(
        ts.path.with_name("tilt.com"),
        "OutputFile",
        f"{ts.path.parent.name}_full_rec.mrc",
    )
    modify_value(
        ts.path.with_name("tilt.com"), "FULLIMAGE", f"{fullimage[0]} {fullimage[1]}"
    )
